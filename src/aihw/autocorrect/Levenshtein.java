...
public class Levenshtein {
  ...
  // The maximum levenshtein distance for consideration
  public static final int MAX_DISTANCE = 5;
  public static final String DICTIONARY_FILE_LOCATION = "";
  ...
  public static LinkedList<String> getOptions(Word toCompare) {
    LinkedList<String> dictionaryList = new LinkedList<String>();
    LinkedList<String> optionList = new LinkedList<String>();
//    if (MAX_DISTANCE < 0) { // Dead code
//      throw new IllegalArgumentException("Distance must be at least 0!");
//    }
    Scanner dictionary = null;
    try {
      dictionary = new Scanner(
          new FileInputStream(new File(DICTIONARY_FILE_LOCATION)));
    ...
    for (int i = 0; i <= 5; i++) {
      while (dictionary.hasNext()) {
        String curr = dictionary.next();
        if (limitedCompare(toCompare.getWord(), curr, i) != -1) {
          optionList.add(curr);
        }
      }
    }
  ...
  /*
   * This implementation of comparation for Levenshtein Distance was pulled from
